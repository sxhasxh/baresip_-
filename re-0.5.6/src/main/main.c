...
//如果flags是0，则继续循环，不是0（文件描述符可读写），则执行下面代码
		if (!flags)
			continue;
		if (re->fhs[fd].fh) {
...
int re_main(re_signal_h *signalh)
{
	...
	if (re->polling) {
		...
		printf("main loop already polling\n");
		return EALREADY;
	}
	...
	DEBUG_INFO("Using async I/O polling method: `%s'\n",
		   poll_method_name(re->method));
	printf("Using async I/O polling method: `%s'\n",
		   poll_method_name(re->method));
